...
class ReolinkAPI {
    ...
    /**
     * Siren sustained manual on/off — alarm_mode "manul", manual_switch 1/0.
     * The official ON/OFF examples send NO `times` field; including `times:1`
     * makes some firmware (e.g. CX820 v3.2.x) play once instead of sustaining,
     * so it is intentionally omitted here.
     */
    setSirenManual(channel, on) {
        const ch = channel ?? this.channel;
        return this._cmdDirect('AudioAlarmPlay',
            { alarm_mode: 'manul', manual_switch: on ? 1 : 0, channel: ch },
            0, { channel: ch });
    }
