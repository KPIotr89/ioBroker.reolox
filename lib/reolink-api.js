...
class ReolinkAPI {
    ...
    /**
     * Siren sustained manual on/off — AudioAlarmPlay, manual_switch 1/0, no `times`
     * (a `times` field makes some firmware play once instead of holding).
     *
     * Firmware disagrees on the alarm_mode spelling: CX810 (v3.1.x) uses "manul",
     * while the v8 HTTP-API guide and newer firmware (CX820 v3.2.x) document "manu".
     * We send "manul" first then "manu" and ignore whichever spelling the firmware
     * rejects, so a single call works across models without per-model detection.
     */
    async setSirenManual(channel, on) {
        const ch = channel ?? this.channel;
        const sw = on ? 1 : 0;
        let ok = false;
        let lastErr = null;
        for (const mode of ['manul', 'manu']) {
            try {
                await this._cmdDirect('AudioAlarmPlay', { alarm_mode: mode, manual_switch: sw, channel: ch }, 0, { channel: ch });
                ok = true;
            } catch (e) { lastErr = e; }
        }
        if (!ok) throw lastErr || new Error('AudioAlarmPlay manual rejected (manul/manu)');
        return { ok: true };
    }
