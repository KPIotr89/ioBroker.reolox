...
class ReolinkAPI {
    ...
    /** Update only the WhiteLed brightness (0-100) and/or mode (0=auto night, 1=manual, 3=schedule). */
    async setWhiteLedConfig(channel, { state, bright, mode }) {
        const ch = channel ?? this.channel;
        const current = await this.getWhiteLed(ch);
        const wl = (current && current.WhiteLed) || current || {};
        wl.channel = ch;
