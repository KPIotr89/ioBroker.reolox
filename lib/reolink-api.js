...
class ReolinkAPI {
    ...
    async _batchCmd(commands) {
        ...
        const run = () => {
            ...
            return this._raw('post', url, body);
        };

        let res = await run();
        // -6 ("please login first") may surface per-entry (data[i].code or
        // data[i].error.rspCode) or as HTTP 401 — re-login once and retry the batch.
        // Without this a server-side token invalidation (NVR reboot) is invisible:
        // every command returns -6 and the caller sees no exception.
        const first = Array.isArray(res.data) ? res.data[0] : res.data;
        const isAuthError = res.status === 401
            || (first && first.code === -6)
            || (first && first.error && first.error.rspCode === -6);
        if (isAuthError) {
            this.token = null;
            await this.login();
            res = await run();
    ...
    // NOTE: GetAudioAlarmV20 returns the config under the `Audio` key on current
    // firmware, so SetAudioAlarmV20 must echo it back under `Audio` (not `AudioAlarmV20`).
    setAudioAlarm(c) { return this._cmd('SetAudioAlarmV20', { Audio: c }, 0); }
    ...
    async setAudioAlarmConfig(channel, { duration, sound }) {
        ...
        const cfg = (current && (current.Audio || current.AudioAlarmV20)) || {};
        cfg.channel = ch;
        if (duration !== undefined) cfg.duration = Number(duration);
        if (sound !== undefined) cfg.sound_index = Number(sound);
        return this._cmd('SetAudioAlarmV20', { Audio: cfg }, 0);
    }
     ...
     *
     * Sends ONLY the fields the caller passed in (plus `channel`). Read-modify-write
     * is intentionally NOT used here — CX810 firmware rejects payloads that contain
     * `mode: 1` with rspCode=-13 ("set config failed"), and round-tripping the whole
     * GetWhiteLed response would always include it. With a minimal payload the
     * camera applies what it can and returns rspCode=200.
     *
     * Mode mapping caveats (verified on CX810 FW v3.1.0.5129):
     *   - mode 0 → AutoNight (accepted)
     *   - mode 1 → Manual    (REJECTED — use mode 3 instead, which is the real
     *                         "manual control" mode on CX-series)
     *   - mode 3 → Schedule / manual ON (accepted)
     */
    async setWhiteLedConfig(channel, { state, bright, mode }) {
        ...
        if (mode !== undefined) {
            // Caller may pass Manual = 1; on CX-series remap to 3 (real manual mode).
            const m = Number(mode);
            wl.mode = m === 1 ? 3 : m;
        }
    ...
    /**
     * Siren timed pulse. Per the official Reolink API the param is FLAT (not wrapped
     * in an AudioAlarmPlay object) and uses alarm_mode="times" with `times` = seconds:
     *   [{cmd:"AudioAlarmPlay", action:0, param:{alarm_mode:"times", manual_switch:0, times:<s>, channel}}]
     */
    triggerSiren(channel, durationSec = 5) {
        const ch = channel ?? this.channel;
        return this._cmdDirect('AudioAlarmPlay',
            { alarm_mode: 'times', manual_switch: 0, times: Number(durationSec) || 5, channel: ch },
            0, { channel: ch });
    }

    /**
     * Siren sustained manual on/off — AudioAlarmPlay, alarm_mode "manul", manual_switch 1/0,
     * no `times` (a `times` field makes some firmware play once). Note: only firmware that
     * supports a held alarm honours this (e.g. CX810 v3.1.x). CX820 v3.2.x accepts the
     * command (rspCode 200) but does NOT hold the siren and cannot stop a pulse early —
     * use control.siren (timed pulse) or control.sirenOnDetect there. ("manu" is rejected
     * with -56 on CX820, so the correct spelling is "manul".)
     */
    setSirenManual(channel, on) {
        const ch = channel ?? this.channel;
        return this._cmdDirect('AudioAlarmPlay', { alarm_mode: 'manul', manual_switch: on ? 1 : 0, channel: ch }, 0, { channel: ch });
    }

    /** Read whether the audio-alarm-on-detection feature is enabled (GetAudioAlarmV20 → Audio.enable). */
    async getAudioAlarmEnabled(channel) {
        const r = await this.getAudioAlarmState(channel);
        const a = (r && (r.Audio || r.AudioAlarmV20)) || {};
        return !!a.enable;
    }

    /**
     * Enable/disable the audio-alarm-on-detection ("armed siren") — SetAudioAlarmV20,
     * preserving the existing schedule. Param key is `Audio` (matches GetAudioAlarmV20).
     */
    async setAudioAlarmEnabled(channel, enabled) {
        const ch = channel ?? this.channel;
        const cur = await this.getAudioAlarmState(ch);
        const audio = (cur && (cur.Audio || cur.AudioAlarmV20)) || {};
        audio.enable = enabled ? 1 : 0;
        if (audio.schedule && audio.schedule.channel === undefined) audio.schedule.channel = ch;
        return this._cmd('SetAudioAlarmV20', { Audio: audio }, 0);
    }
