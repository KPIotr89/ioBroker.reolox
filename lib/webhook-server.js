...
class WebhookServer {
    ...
    constructor(opts) {
        ...
        // Allowlist supports `auto` (derive from camera hosts) and explicit IPs at the
        // same time. Back-compat: a bare 'auto' string still means auto-only.
        if (opts.ipAllowlist === 'auto') {
            this.allowAuto = true;
            this.ipAllowlist = [];
        } else {
            this.ipAllowlist = Array.isArray(opts.ipAllowlist) ? opts.ipAllowlist : [];
            this.allowAuto = !!opts.ipAllowlistAuto;
        }
    ...
    _ipAllowed(ip) {
        // `auto` (camera hosts) and the explicit list are combinable, so a setup can
        // keep auto camera detection AND add e.g. an admin workstation.
        if (this.allowAuto) {
    ...
    _ipAllowedForControl(ip) {
        // Loopback is always allowed for control so the endpoint can be exercised
        // from the ioBroker host itself (curl, local scripts).
        if (ip === '127.0.0.1' || ip === '::1') return true;
        return this._ipAllowed(ip) || this.extraAllowed.includes(ip);
    }

    /** Sources allowed to skip the shared secret on the control endpoint:
     *  the ioBroker host (loopback) and explicitly trusted IPs (the Miniserver). */
    _isTrustedForControl(ip) {
        return ip === '127.0.0.1' || ip === '::1' || this.extraAllowed.includes(ip);
    }
    ...
    _handleControl(req, res, url, sourceIp) {
        ...
        // Strongly-trusted sources (Miniserver IP, loopback) skip the secret;
        // any other allowed IP must still present it.
        if (!this._isTrustedForControl(sourceIp) && !this._secretOk(req, url)) {
