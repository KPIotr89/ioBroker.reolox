...
class CapabilityCache {
    ...
    get(host, port, user, firmVer) {
        try {
            ...
            // Invalidate when the firmware changed since the entry was written —
            // GetAbility can differ across firmware revisions.
            if (firmVer && raw.firmVer && raw.firmVer !== firmVer) return null;
     ...
     */
    set(host, port, user, data, firmVer) {
        try {
            const p = this._path(this._key(host, port, user));
            fs.writeFileSync(p, JSON.stringify({ ts: Date.now(), firmVer: firmVer || '', data }));
        } catch (e) {
